...
def run():
    parser = argparse.ArgumentParser(prog="pydo", description="A simple command-line todo list manager.")
    # This is a top-level flag that affects context for most sub-commands.
    parser.add_argument("-g", "--global", dest="is_global", action="store_true", help="Force operation on the global todo list.")
    ...
    # Command: sync #
    #################
    parser_sync = subparsers.add_parser("sync", help="Synchronize list with google tasks")
    parser_sync.set_defaults(func=handlers.handle_sync)

    ###################
    # Command: status #
    ###################
    parser_status = subparsers.add_parser("status", help="Show the currently active list (local or global).", aliases=["st"])
    ...
    parser_list.add_argument("-f", "--focus", action="store_true", help="Show only focused tasks.")
    ...
    # Command cls #
    ###############
    parser_clearlist = subparsers.add_parser("cls", help="Clear screen and list tasks.", aliases=["c"])
    parser_clearlist.add_argument("-a", "--all", action="store_true", help="Show all tasks, including completed ones.")
    parser_clearlist.add_argument("-f", "--focus", action="store_true", help="Show only focused tasks.")
    parser_clearlist.add_argument("--done", action="store_true", help="Show only completed tasks.")
    ...
    parser_focus = subparsers.add_parser("focus", aliases=["f"], help="Toggle focus on task(s)")
    parser_focus.add_argument("task_ids", nargs="+", type=int, help="The ID(s) of the task(s) to add focus to.")
    parser_focus.set_defaults(func=handlers.handle_focus)

    #####################
    # Command: done / d #
    #####################
    parser_done = subparsers.add_parser("done", help="Mark task(s) as done.", aliases=["d"])
    ...
    # Command hide / h #
    ####################
    parser_hide = subparsers.add_parser("hide", help="Mark task(s) as hidden.", aliases=["h"])
    parser_hide.add_argument("task_ids", nargs="+", type=int, help="The ID(s) of the task(s) to mark as hidden.")
    parser_hide.set_defaults(func=handlers.handle_hide)

    ##################
    # Command unhide #
    ##################
    parser_unhide = subparsers.add_parser("unhide", help="Remove hidden flag from tasks.", aliases=["uh"])
    parser_unhide.add_argument("task_ids", nargs="+", type=int, help="The ID(s) of the task(s) remove hidden flag.")
    parser_unhide.set_defaults(func=handlers.handle_unhide)

    ###################
    # Command: undone #
    ###################
    parser_undone = subparsers.add_parser("undone", aliases=["u"], help="Mark task(s) as not done.")
